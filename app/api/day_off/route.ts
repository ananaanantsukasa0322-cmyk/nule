...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.driver_id && body.date) {
      const { error } = await supabase.from('day_off').upsert(
        { target_date: body.date, driver_id: body.driver_id },
        { onConflict: 'target_date,driver_id' }
      )
      if (error) {
        await supabase.from('day_off').insert({ target_date: body.date, driver_id: body.driver_id })
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    if (body.driver_id && body.date) {
      await supabase.from('day_off').delete()
